...
在本節中，我們先求解另一個常見的揹包問題：完全揹包，再瞭解它的一種特例：零錢兌換。

## 完全揹包問題

!!! question

    給定 $n$ 個物品，第 $i$ 個物品的重量為 $wgt[i-1]$、價值為 $val[i-1]$ ，和一個容量為 $cap$ 的揹包。**每個物品可以重複選取**，問在限定揹包容量下能放入物品的最大價值。示例如下圖所示。

![完全揹包問題的示例資料](unbounded_knapsack_problem.assets/unbounded_knapsack_example.png)

### 動態規劃思路

完全揹包問題和 0-1 揹包問題非常相似，**區別僅在於不限制物品的選擇次數**。

- 在 0-1 揹包問題中，每種物品只有一個，因此將物品 $i$ 放入揹包後，只能從前 $i-1$ 個物品中選擇。
- 在完全揹包問題中，每種物品的數量是無限的，因此將物品 $i$ 放入揹包後，**仍可以從前 $i$ 個物品中選擇**。

在完全揹包問題的規定下，狀態 $[i, c]$ 的變化分為兩種情況。

- **不放入物品 $i$** ：與 0-1 揹包問題相同，轉移至 $[i-1, c]$ 。
- **放入物品 $i$** ：與 0-1 揹包問題不同，轉移至 $[i, c-wgt[i-1]]$ 。
...
這個走訪順序與 0-1 揹包正好相反。請藉助下圖來理解兩者的區別。

=== "<1>"
    ![完全揹包問題在空間最佳化後的動態規劃過程](unbounded_knapsack_problem.assets/unbounded_knapsack_dp_comp_step1.png)
...
揹包問題是一大類動態規劃問題的代表，其擁有很多變種，例如零錢兌換問題。
...
**零錢兌換可以看作完全揹包問題的一種特殊情況**，兩者具有以下聯絡與不同點。

- 兩道題可以相互轉換，“物品”對應“硬幣”、“物品重量”對應“硬幣面值”、“揹包容量”對應“目標金額”。
- 最佳化目標相反，完全揹包問題是要最大化物品價值，零錢兌換問題是要最小化硬幣數量。
- 完全揹包問題是求“不超過”揹包容量下的解，零錢兌換是求“恰好”湊到目標金額的解。
