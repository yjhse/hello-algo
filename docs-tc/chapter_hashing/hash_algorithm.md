...
前兩節介紹了雜湊表的工作原理和雜湊衝突的處理方法。然而無論是開放定址還是鏈式地址，**它們只能保證雜湊表可以在發生衝突時正常工作，而無法減少雜湊衝突的發生**。

如果雜湊衝突過於頻繁，雜湊表的效能則會急劇劣化。如下圖所示，對於鏈式地址雜湊表，理想情況下鍵值對均勻分佈在各個桶中，達到最佳查詢效率；最差情況下所有鍵值對都儲存到同一個桶中，時間複雜度退化至 $O(n)$ 。
...
對於密碼學的相關應用，為了防止從雜湊值推匯出原始密碼等逆向工程，雜湊演算法需要具備更高等級的安全特性。
...
- **異或雜湊**：將輸入資料的每個元素透過異或操作累積到一個雜湊值中。
...
不難發現，以上介紹的簡單雜湊演算法都比較“脆弱”，遠遠沒有達到雜湊演算法的設計目標。例如，由於加法和異或滿足交換律，因此加法雜湊和異或雜湊無法區分內容相同但順序不同的字串，這可能會加劇雜湊衝突，並引起一些安全問題。
...
- 物件的雜湊值基於其記憶體地址生成。透過重寫物件的雜湊方法，可實現基於內容生成雜湊值。
...
在許多程式語言中，**只有不可變物件才可作為雜湊表的 `key`** 。假如我們將列表（動態陣列）作為 `key` ，當列表的內容發生變化時，它的雜湊值也隨之改變，我們就無法在雜湊表中查詢到原先的 `value` 了。

雖然自定義物件（比如鏈結串列節點）的成員變數是可變的，但它是可雜湊的。**這是因為物件的雜湊值通常是基於記憶體地址生成的**，即使物件的內容發生了變化，但它的記憶體地址不變，雜湊值仍然是不變的。
